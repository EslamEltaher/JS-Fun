...
function MineSweeperCtrl($scope, $timeout, $interval, $timeout) {
    ...
    function analyzeCell(cell, fun) {
        ...
        if(cell.bombCount == hidden) {
            ...
            var indetrminedBombs = cell.bombCount - marked;
            var indetrminedCells = hidden - marked;
            var guess = indetrminedBombs/indetrminedCells;

            var guessArray = [];
            aroundCell(cell.i, cell.j, function(cell) {
                if(!cell.show && !cell.isMarked)
                    guessArray.push(cell);
            });
            guessArray.sort((a,b) => a.guess < b.guess);
            var g = indetrminedBombs;
            for(var i = 0; i < guessArray.length; i++)
                if(g == 0) {
                    showCell(cell.i, cell.j)
                } else {
                    g -= (cell.guess ? cell.guess : 0);
                }
            aroundCell(cell.i, cell.j, function(cell) {
                if(!cell.show && !cell.isMarked)
                    cell.guess = cell.guess > guess ? cell.guess : guess;
            });
