...
function ACT() {
    ...
    setInterval(function(){
        ...
        loopCanvas(cvs,function(p){
            ...
            var fac = colors ?  1/colors : 0;

            if(k-->0)   console.log('R: ',R, ' G: ', G, ' B: ', B, ' colors: ', colors);
            var r = Math.floor((255 - R) * fac + G * fac + B * fac );
            var g = Math.floor((255 - G) * fac + R * fac + B * fac );
            var b = Math.floor((255 - B) * fac + R * fac + G * fac );
...
function drawPoint(x,y,c) {
    ...
    return draw.drawCircle(x.x,x.y,5, { fillStyle: c, strokeStyle : '#FFFFFF' });
}
